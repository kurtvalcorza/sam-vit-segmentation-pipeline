"""Static release-asset validation for the SAM ViT-B promptable-segmentation DIMER pipeline.
...
EXPECTED_MODEL_ID = "facebook/sam-vit-base"
...
# Additional 40-hex revisions a document may legitimately cite: the ADE20K parquet-conversion commit
# the sample is pinned to (samples.py CORPUS_REVISION).
KNOWN_SHAS: frozenset[str] = frozenset(("e660d866a1351c70bcf07925d2600b60bd6e3bc3",))
...
EXPECTED_OUTPUTS = (
    "outputs/sam_vit_segmentation_train.csv",
    "outputs/sam_vit_segmentation_input_manifest.json",
    "outputs/sam_vit_segmentation_evaluation_report.json",
    "outputs/sam_vit_segmentation_mask_frozen.png",
    "outputs/sam_vit_segmentation_mask_adapted.png",
    "outputs/sam_vit_segmentation_examples",
    "outputs/sam_vit_segmentation_adapter",
    "outputs/sam_vit_segmentation_result.json",
)
...
CODE_MARKERS = (
    ...
    "print({'ceilings': {'MIN_IMAGE_SIDE': MIN_IMAGE_SIDE, 'MAX_IMAGE_SIDE': MAX_IMAGE_SIDE, 'MAX_PROMPTS': MAX_PROMPTS, 'NUM_MULTIMASK_OUTPUTS': NUM_MULTIMASK_OUTPUTS, 'MASK_THRESHOLD': MASK_THRESHOLD, 'MIN_RECORDS': MIN_RECORDS, 'MAX_RECORDS': MAX_RECORDS",
    ...
    "'scores_above_one': [i for i, v in enumerate(result['iou_scores']) if v > 1.0]",
    ...
    "'corpus': {'name': CORPUS_NAME, 'repo': CORPUS_REPO, 'revision': CORPUS_REVISION, 'file': CORPUS_FILE, 'license': CORPUS_LICENSE, 'shard_bytes': CORPUS_BYTES",
    "transformers.__version__",
    "'device': pipe.device",
)
# Profile-specific learner-facing statements.
MARKDOWN_MARKERS = (
    "**Capability:** promptable image segmentation (one point click or one box → one object's mask) and bounded supervised fine-tuning of the mask decoder",
    "**Apache-2.0** licence",
    "**adaptation with labelled targets**",
    "the predicted IoU is a learned, uncalibrated ranking score, not a probability",
    "**can exceed 1.0**",
    "**0.564**",
    "**mixed** point and box prompts",
    "**point-prompt IoU**",
    "**box-fill**",
    "**centre-disk**",
    "**binary cross-entropy plus a soft Dice term**",
    "**no dispersion estimate**",
    "**Snapshot note:**",
    "automatic \"segment everything\" mask generation, text prompts",
...
EXTERNAL_WEIGHT_BYTES: dict[int, str] = {
    16_248_448: "sam-vit adapter.safetensors from the tutorial run (120 tensors)",
}
